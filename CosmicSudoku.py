...
def printList(theList):
    for row,col,board in theList:
        print(" " + str(row)+","+str(col))


def solve_sudoku_no_recursion(incboard):
    stack = [(int(0), int(0),incboard)]  # Stack to store (row, col) pairs
    while stack:
        lastitem = len(stack)
        row, col, board = stack[lastitem-1]
        print("Stack Item  " + str(row) + " col " + str(col) + " with " + str(board[row][col]))
        print("Stack:")
        printList(stack)
        
        if board[row][col] != 0:
             print(" Already Filled at  " + str(row) + " col " + str(col) + " with " + str(startValue))
        else:
            print("Seeking next square to fill")
        # Find an empty location
            while row < 9 and incboard[row][col] != 0:
                row, col = (row, col+1) if col < 8 else (row + 1, 0)

        startValue = board[row][col]
        if startValue == 0: startValue = 1
        print("  Attempting Row " + str(row) + " col " + str(col) + " from " + str(startValue))

        if row == 9:  # Solved if we reach the end
            return True

        # Try placing a number from 1 to 9
        for num in range(startValue, 10):
            print("   " + str(row) + ":" + str(col) +  " Trying number " + str(num))
#             print_sudoku(incboard)
            display_sudoku(sudoku_board)
            time.sleep(0.2)
            if is_valid(board, row, col, num):
                print("   " + str(num) + " was valid at " + str(row) + ":" + str(col) + "; Setting board, adding to stack")
                board[row][col] = num
                stack.append((row, col, board))
                col = 0
                break
        else:  # No valid number was found, backtrack
            print("No valid number found for " + str(row) + ":" + str(col))
            board[row][col] = 0
            row,col,board = stack[len(stack)-1]
            print(" Going back to " + str(row) + ":" + str(col))
            display_sudoku(sudoku_board)
            time.sleep(0.2)
